...
BOOST_AUTO_TEST_SUITE(InitializeMatrix)
    ...
    BOOST_AUTO_TEST_CASE(InitializeMatrix_TestCopyConstructor_NoError)
    {
      ...
      BOOST_REQUIRE_CLOSE(1.1, new_maxtrix[0][0]->getCenter().x, accuracy);
      BOOST_REQUIRE_CLOSE(2.2, new_maxtrix[0][0]->getCenter().y, accuracy);
      BOOST_REQUIRE_CLOSE(M_PI * 3.3 * 3.3, new_maxtrix[0][0]->getArea(), accuracy);

      BOOST_REQUIRE_EQUAL(1, old_maxtrix.getRowNumber());
      BOOST_REQUIRE_CLOSE(1.1, old_maxtrix[0][0]->getCenter().x, accuracy);
      BOOST_REQUIRE_CLOSE(2.2, old_maxtrix[0][0]->getCenter().y, accuracy);
      BOOST_REQUIRE_CLOSE(M_PI * 3.3 * 3.3, old_maxtrix[0][0]->getArea(), accuracy);
    }
...
BOOST_AUTO_TEST_SUITE(GetShapeinMatrix)
    BOOST_AUTO_TEST_CASE(GetShapeinMatrix_GetFirstShape_NoError)
    {
      ...
      jianing::Shape::ShapePtr get_circle = maxtrix[0][0];
    ...
    BOOST_AUTO_TEST_CASE(GetShapeinMatrix_UseFunctiongetShape_NoError)
    {
      ...
      BOOST_CHECK_EQUAL(first_circle, maxtrix[0][0]);
      BOOST_CHECK_EQUAL(rectangle, maxtrix[1][0]);
      BOOST_CHECK_EQUAL(sec_circle, maxtrix[0][1]);
    }
    ...
    BOOST_AUTO_TEST_CASE(GetShapeinMatrix_UseFunctiongetShapeOnWrongValue_ThrowError)
    {
      ...
      BOOST_CHECK_THROW(maxtrix[0][2], std::out_of_range);
      BOOST_CHECK_THROW(maxtrix[1][0], std::out_of_range);
      BOOST_CHECK_THROW(maxtrix[1][2], std::out_of_range);
    }
...
BOOST_AUTO_TEST_SUITE(AddShapeinMatrix)
    BOOST_AUTO_TEST_CASE(AddShapeinMatrix_AddCorrectShape_NoError)
    {
      ...
      BOOST_CHECK_CLOSE(1.1, maxtrix[0][0]->getCenter().x, accuracy);
      BOOST_CHECK_CLOSE(2.2, maxtrix[0][0]->getCenter().y, accuracy);
      BOOST_CHECK_CLOSE(M_PI * 3.3 * 3.3, maxtrix[0][0]->getArea(), accuracy);
    }
