...
/*
 * Note: This function removes this character while matching for the next character in the input stream.
 */
std::istream& jianing::B4::rmwAndMatchSign(std::istream& istr, const char& c_sign)
{
  if (!istr.good())
  {
    return istr;
...
std::istream& jianing::B4::operator>>(std::istream& istr, DataStruct& data)
{
  ...
  DataStruct tmp_data = readData(istr);
