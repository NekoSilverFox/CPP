...
BOOST_AUTO_TEST_SUITE(RotateRectangle)

  BOOST_AUTO_TEST_CASE(RotateRectangle_InPositiveDirection_CorrectWigthHeightXYAfterRotate)
  {
    jianing::Rectangle rectangle {{2.3, 4.6, 5.7, 7.5}};

    rectangle.rotate(27);

    BOOST_CHECK_EQUAL(5.7, rectangle.getCenter().x);
    BOOST_CHECK_EQUAL(7.5, rectangle.getCenter().y);
    BOOST_CHECK_EQUAL(2.3, rectangle.getWidth());
    BOOST_CHECK_EQUAL(4.6, rectangle.getHeight());
    BOOST_CHECK_CLOSE(2.3 * 4.6, rectangle.getArea(), accuracy);
  }

BOOST_AUTO_TEST_SUITE_END()

// =====================test rotate rectangle(FrameRect)=====================
BOOST_AUTO_TEST_SUITE(TestFrameRectAfterRotateRectangle)

  BOOST_AUTO_TEST_CASE(TestFrameRectAfterRotateRectangle_InPositiveDirection_CorrectFrameAfterRotate)
  {
    jianing::Rectangle rectangle {{4.0, 5.0, 2.0, 3.0}};

    rectangle.rotate(60);

    BOOST_CHECK_EQUAL(2.0, rectangle.getFrameRect().pos.x);
    BOOST_CHECK_EQUAL(3.0, rectangle.getFrameRect().pos.y);
    BOOST_CHECK_CLOSE(4.0 * std::fabs(cos(M_PI * 60 / 180)) + 5.0 * std::fabs(sin(M_PI * 60 / 180)), rectangle.getFrameRect().width, accuracy);
    BOOST_CHECK_CLOSE(5.0 * std::fabs(cos(M_PI * 60 / 180)) + 4.0 * std::fabs(sin(M_PI * 60 / 180)), rectangle.getFrameRect().height, accuracy);
  }
